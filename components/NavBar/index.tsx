    ...
    <ul className="flex w-full select-none flex-row items-center justify-center gap-8 max-lg:hidden lg:gap-16">
      ...
      <li className="relative">
        <Link
          ...
          className="navLink"
          onClick={() => posthog?.capture('nav_link_clicked', { destination: 'shop' })}
        >
          <span>Shop</span>
        </Link>
...
const HamburguerMenu = () => {
  ...
  return (
    <DropdownMenu
      ...
      onOpenChange={(value) => {
        setIsOpen(value)
        posthog?.capture('hamburger_menu_toggled', { open: value })
      }}
    >
      <DropdownMenuTrigger asChild>
        <button
          ...
          aria-haspopup="menu"
        >
          <MenuIcon className="pointer-events-none size-full transition-transform duration-200 ease-out group-hover:scale-[1.1]" />
